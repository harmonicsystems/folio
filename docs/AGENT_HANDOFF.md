...
| ~~Decodability calibration~~ — **DIAGNOSED 2026-06-26, ADR 0004** | Root cause verified: DH-inside-sight-words ("the/that/this"; DH ease 0.30) dominates; length normalized away; production norms used as decoding proxy. Cross-band monotonicity judged the WRONG target (read-aloud bands aren't decoded by the child) → claims scoped in guide/types/phonology docstring instead of formula tuning | Resolved as docs. Next real step: **CMU dict expansion** (~2k words) — it gates the honest sight-word-excluded variant, since non-sight tokens are currently ~97–100% grapheme-guessed. Destination construct (recorded, not started): GPC-based percent-decodable metric |
