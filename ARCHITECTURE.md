...
- [x] Type-token ratio
- [x] Vocabulary tier classification (Tier 1 vs reach) — **shipped 2026-06-15**: Tier 1 = Dale–Chall familiar list ∪ Dolch/Fry sight words; `tier1Coverage` + `tier2Words` populated, reach words flagged `tier-2`. Tier-3 detection still later (needs a frequency band; documented in tiers.ts)
...
- [x] Decodability calibration pass against the full corpus — diagnosed 2026-06-26; formula deliberately kept, claims scoped instead (ADR 0004); sight-word-excluded scoring gated on CMU dict expansion
...
- **~~Tier 1 word list sourcing.~~ RESOLVED 2026-06-15.** Beck/McKeown/Kucan publish no canonical Tier 1 list, so we adopted the **Dale–Chall (1995) familiar-word list** as a sourced child-normed proxy (∪ Dolch/Fry sight words). `tier1Coverage` and `tier2Words` now do real work. Remaining: tier-2 vs tier-3 isn't split (needs a frequency band wider than Dale–Chall), and inflected forms / proper nouns read as reach words (stem-aware classification → `morphologically-complex` is future work). See `data/dale-chall.ts`, `vocabulary/tiers.ts`, and SOURCES.md (Vocabulary).
- **~~Decodability calibration.~~ DIAGNOSED 2026-06-26 (ADR 0004) — formula deliberately unchanged.** The earlier causal guess here (syllable weights under-counting complex shapes) was wrong. The measured dominant mechanism: "the/that/this" are built on DH — the latest-rated phoneme (age 6.5 → ease 0.30) — and 100% of DH occurrences in all 10 fixtures sit inside sight words, so high-repetition sight-word text is scored as if children sound out words they are taught to recognize whole. Secondary mechanisms: per-position averaging normalizes word length away, and acquisition ages are speech-production norms used as a decoding proxy. Cross-band monotonicity was judged the wrong calibration target (read-aloud bands aren't decoded by the child), so the resolution is claim-scoping in the docs/guide, not formula tuning. The honest formula fix — scoring only non-sight-word tokens — is **gated on CMU dict expansion**: today's non-sight tokens are ~97–100% grapheme-guessed. Independent of all of this, a vowel-acquisition citation would let us drop the "all vowels = 3.0" convention.
